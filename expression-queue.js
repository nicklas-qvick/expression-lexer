...
export default class ExpressionQueue
{
    ...
    appendRule(expressionRule)
    {
        return this.expressionRules.push(expressionRule)
    }

    findRule(expressionRule)
    {
        return this.expressionRules.at(expressionRule)
    }

    generateTable()
    {
        const expressionStates = this.expressionRules.splice()

        const expressionRules = this.expressionRules.slice()

        const expressionValues = this.expressionRules.map(Expression.createTermination)

        expressionRules.unshift(Expression.createTermination())

        expressionValues.unshift(Expression.createTermination())

        const expressionRule = expressionRules.reduceRight(ExpressionNode.composeProduction)
        ...
        {
            const expressionRanges = expressionState.getPartition(new ExpressionRange(0, 255))

            for(const expressionRange of expressionRanges)
            {
                const expressionNode = expressionState.getDerivation(expressionRange)
                ...
                if(expressionInstance)
                {
                    ...
                    {
                        expressionState.setRoute(expressionByte, expressionInstance)
                    }
                }
            }
        }
